...
> [!IMPORTANT]
> The settings UI is only available in [Windows Terminal Preview](https://aka.ms/terminal-preview). To enable the settings UI with the command palette or keyboard, you need to add the `"openSettings"` action to the `"actions"` array of your settings.json file. Select Settings from the drop-down menu inside Terminal, which will open the settings.json file. Find the `"actions":` section of the file and add this line: `{ "command": { "action": "openSettings", "target": "settingsUI" }, "keys": "ctrl+shift+," },`. Restart Terminal and select `Ctrl`+`Shift`+`,` to open the new settings UI menu.


Detailed instructions on how to enable the settings UI can be found on the [Troubleshooting page](./../troubleshooting.md#open-the-settings-ui).
